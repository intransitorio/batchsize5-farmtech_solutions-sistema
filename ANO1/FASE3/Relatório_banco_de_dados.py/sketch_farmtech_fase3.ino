#include "DHT.h"

#define DHTPIN 15
#define DHTTYPE DHT22
#define PIN_LDR 34
#define PIN_BTN_N 12
#define PIN_BTN_P 13
#define PIN_BTN_K 14
#define PIN_RELE 2

DHT dht(DHTPIN, DHTTYPE);

bool deficienciaN = false;
bool deficienciaP = false;
bool deficienciaK = false;

int lastN = HIGH;
int lastP = HIGH;
int lastK = HIGH;

unsigned long tempoAnterior = 0;
const long intervalo = 2000;

int horaSimulada = 0;
int diaSimulado = 1;

void setup() {

  Serial.begin(115200);
  delay(1000);

  pinMode(PIN_BTN_N, INPUT_PULLUP);
  pinMode(PIN_BTN_P, INPUT_PULLUP);
  pinMode(PIN_BTN_K, INPUT_PULLUP);
  pinMode(PIN_RELE, OUTPUT);

  dht.begin();

  Serial.println("\n--- FarmTech: Monitoramento de Deficiencias ---");
  Serial.println("Timestamp,Umid,pH,Defic_N,Defic_P,Defic_K,Bomba");
}

void loop() {

  int currN = digitalRead(PIN_BTN_N);
  int currP = digitalRead(PIN_BTN_P);
  int currK = digitalRead(PIN_BTN_K);

  if (lastN == HIGH && currN == LOW) {
    deficienciaN = !deficienciaN;
    delay(50);
  }

  if (lastP == HIGH && currP == LOW) {
    deficienciaP = !deficienciaP;
    delay(50);
  }

  if (lastK == HIGH && currK == LOW) {
    deficienciaK = !deficienciaK;
    delay(50);
  }

  lastN = currN;
  lastP = currP;
  lastK = currK;

  unsigned long tempoAtual = millis();

  if (tempoAtual - tempoAnterior >= intervalo) {

    tempoAnterior = tempoAtual;

    float umid = dht.readHumidity();

    if (isnan(umid)) {
      umid = 53.0;
    }

    int ldrRaw = analogRead(PIN_LDR);

    float ph = map(ldrRaw, 0, 4095, 0, 14);

    bool precisaAgua = (umid < 60.0);

    bool soloCarente =
      (deficienciaN ||
       deficienciaP ||
       deficienciaK);

    bool phOk =
      (ph >= 5.5 && ph <= 6.5);

    bool riscoEncharcamento =
      (umid >= 75.0);

    bool ligarBomba =
      (phOk &&
       !riscoEncharcamento &&
       (precisaAgua || soloCarente));

    digitalWrite(
      PIN_RELE,
      ligarBomba ? HIGH : LOW
    );

    String timestamp = "2026-05-";

    if (diaSimulado < 10) {
      timestamp += "0";
    }

    timestamp += String(diaSimulado);
    timestamp += " ";

    if (horaSimulada < 10) {
      timestamp += "0";
    }

    timestamp += String(horaSimulada);
    timestamp += ":00";

    Serial.print(timestamp);
    Serial.print(",");

    Serial.print(umid);
    Serial.print(",");

    Serial.print(ph);
    Serial.print(",");

    Serial.print(deficienciaN);
    Serial.print(",");

    Serial.print(deficienciaP);
    Serial.print(",");

    Serial.print(deficienciaK);
    Serial.print(",");

    Serial.println(ligarBomba ? 1 : 0);

    horaSimulada += 3;

    if (horaSimulada >= 24) {
      horaSimulada = 0;
      diaSimulado++;
    }
  }
}
